...
C Печать номеров узлов, входящих в текущую зону (по линиям)
   ...
   49 FORMAT(//1X,'REGION NODE NUMBERS S'/)
...
* 33   CONTINUE
   ...
   16 CONTINUE
      NELEM=NEL
       IF(IPR(7)) then
       WRITE(6,555)INRG,MAXNP,NEL
555    FORMAT(/,20X,'Характеристики сетки B',/5X,'ЧИСЛО ЗОН',I3,
     > 2X,'KOЛИЧЕСТBO УЗЛOB',I4,2X,'KOЛИЧЕСТBO ЭЛЕМЕНTOB',I4)       
        endif
       IF(IPR(7)) WRITE(6,51)NBW*2-1
51     FORMAT(/,15X,'Полуширина ленты НЕУПОРЯДОЧЕННОЙ матрицы  ',I5)
