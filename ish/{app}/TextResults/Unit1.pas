...
'23. Распределение нагрузки по узлам. (FORCE)', '24. Различные схемы хранения матриц', '25. Характеристики модфицированной профильной схемы хранения', '26.  Сообщения о работе программ', '27.  Распределение памяти для профильного метода XENV (MAIN)',
'28. Размер оболочки, ширина ленты матрицы (MAIN)', '29. Резерв', '30. Резерв','31.  Перемещения узлов (MAIN)','32.  Напряжения в КЭ (STRDD)');
   ...
   ResultSets:array [1..2*MsgCount] of String=
     ('Hello!','*****Finish DATA',
     '*****Start  GRIDDM','*****Finish GRIDDM',
     'CONNECTIVITY DATA','*****Finish GRIDDM',
   'REGION   1  ****', '*****Finish GRIDDM',
   'NEL   NODE NUMBERS','*****Finish GRIDDM',
   'COORDINATES NOTES OF REGION','*****Finish GRIDDM',
   'Характеристики сетки','*****Finish GRIDDM',
    '*****Start PRNTDD','*****Finish PRNTDD',
    '*****Start PRNTDD','*****Finish PRNTDD',
    '*****Start RENMDD','*****Start STSM in RENMDD',
   'СТРУКТУРА СМЕЖНОСТИ', '*****Start GENRCM in RENMDD',
   '+OLD+ +NEW+ +OLD+ +NEW+ +OLD+ +NEW+','*****Finish RENMDD',
   '+NEW+ +OLD+ +NEW+ +OLD+ +NEW+ +OLD+','*****Finish RENMDD',
   'НОВАЯ НУМЕРАЦИЯ УЗЛОВ (ЭЛЕМЕНТ И ЕГО УЗЛЫ)   MACCИB <<NOP>>','*****Start BOUND',
   '*****Start REGULARIZATION in GRIDDM','*****Finish  REGULARIZATION in GRIDDM',
   '*****Finish ESFCT in RCMSLV','*****Start  ELSLV in RCMSLV',
   'КОНТРОЛЬНАЯ ПЕЧАТЬ ПАРАМЕТРОВ ЗАДАЧИ','*****Start FNENDD',
   'Hello!','****good luck****',
   ' KOOPДИHATЫ УЗЛOB','*****Start FNENDD',
   ' NOP ПЕРЕД FORMDD','*****Finish FORMDD',
   '*****Start BOUND','*****Finish BOUND',
   '*****Start FORCE','PACПPEДEЛEHИE HAГPУЗOK',
   'PACПPEДEЛEHИE HAГPУЗOK','*****Finish FORCE',
   '*****Start SCHEMA','*****Finish SCHEMA',
   'begin memory','end memory',
   'Hello!','****good luck****',
   '*****Finish FNENDD','begin memory',
   '*****Finish FNENDD','begin memory',
   'Hello!','****good luck****',
   'Hello!','****good luck****',
   'ПEPEMEЩEHИЯ УЗЛOB','*****Start STRSDD',
