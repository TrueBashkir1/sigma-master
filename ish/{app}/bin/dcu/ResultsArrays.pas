...
//класс для хранения массива точек, по которым строятся зоны.
//Давыдова Ю.А. май 2011г.
  ZoneContourPoints = CLASS
  PROTECTED
    //массив хранит для 20 возможных зон по 8 точек, которые задаются 2 координатами
    ArrayOfPoints : ARRAY [1..20,1..16] OF Double;
    NumberOfZones : Shortint;
    //задает точки, по которым строится контур зоны под определенным номером
  ...
  PUBLIC
    ...
    //возвращает массив координат для построения контура зоны с определенным номером
...
VAR
  ...
  Materials              : array[1..3] of TMaterial; // Добавляем материалы (kotov)
...
BEGIN
  ...
  AssignFile(F,Path);
  Reset(F);
//надо пройти через несколько меток -1, чтобы добраться до нужных результатов
  n:=0;
  while not EOF(F) do begin
    MyREAD (ADDR(n), 2);
    IF n = 65535 THEN BREAK;
  ...
  //считываем количество зон, на которые разбита пластина
  n:=0;
  while not EOF(F) do begin
    MyREAD (ADDR(n), 2);
    if n=65535 then break;
...
//считываем точки, задающие эти зоны
  k:=0;
  while not Eof(F) do begin
    inc(k);
    for i := 1 to 16 do begin
