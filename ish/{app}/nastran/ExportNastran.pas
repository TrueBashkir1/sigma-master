unit ExportNastran;
//Last Edited By Ivan Vavilov 2013 year
interface
...
//By Ivan Vavilov 2013 year
procedure getProperty(var Nastran:TextFile; Path: String);
var  F: TextFile;
     S1,S2: String;
	 i,k: Integer;
begin
    AssignFile(F,Path+'Nastran.ns0');
    Reset(F);
	i:=0;
	k:=0;
  WHILE NOT Eof(F) DO BEGIN
	...
	begin;
		i:=0;
		k:=k+1;
		S2:='$ FEMAP Property ' + IntToStr(k) + ' : Untitled';
		Writeln(Nastran,S2);
		S2:='PCOMP          ' + IntToStr(k) + '              0.                                        +       ';
		Writeln(Nastran,S2);
		S2:='+              ' + IntToStr(k) + ',' + S1 + ',0.,YES';      
		Writeln(Nastran,S2);
    end;
  end;
    CloseFile(F);
...
procedure getMaterial(var Nastran:TextFile; Path: String);
var  F: TextFile;
     S1,S2,S3,S4: String;//S1 - E, S2 - mu, S3 - sigma
	 i: Integer;
begin
	i:=0;
    AssignFile(F,Path+'Nastran.ns0');
  ...
  WHILE NOT Eof(F) DO BEGIN
    i:=i+1;
	S4:= '$ FEMAP Material ' + IntToStr(i) + ' : Untitled';
	Writeln(Nastran,S4);
	...
	S4:= 'MAT1           ' + IntToStr(i) + ',' + S1 + ',0,' + S2 + ',0,0,0,0,                               +';
	Writeln(Nastran,S4);
	S4:= '+         ' + S3;
	Writeln(Nastran,S4);
...
procedure NElements_angle(var Handle:TextFile; Path: String);{ === Запись Элементов === }
var
	 F: TextFile;
	 S1,S2: String;
     Element: TOneElement;
...
begin
     ...
     WHILE NOT Eof(F) DO BEGIN
           Readln(F, S1);
		   Element:= Elements_Result.GetElement(i+1);
           Node1:=Nodes_Result.GetNode(Element.Node1);
           Node2:=Nodes_Result.GetNode(Element.Node2);
           Node3:=Nodes_Result.GetNode(Element.Node3);
		   ...
		   S2:='CTRIA3     '+S1+', '+toPoint(FloatToStrF(-angle,ffNumber,3,3));
...
begin
	 ...
	 getProperty(Handle,FilePath);
	 getMaterial(Handle,FilePath);
     NKoords(Handle);
     Elements_Result:=TElements.Create;
     Elements_Result.LoadElements(FilePath+'RESULT2.BIN');
//	 New_NElements(Handle,FilePath);
	 NElements_angle(Handle,FilePath);
